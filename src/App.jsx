...
function SpendUpload({db,owners}){
  ...
  const saveMapping=async()=>{
    ...
    try{
      if(existing){
        // Ask to replace — if user cancels, do nothing at all
        const replace=confirm(`A saved mapping named "${existing.name}" already exists. Replace it?`);
        if(!replace) return; // user cancelled — stop here, don't save anything
        // Delete the old one first, then insert fresh
