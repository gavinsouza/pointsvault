...
const sbtn={ ...pbtn, background:"rgba(52,211,153,0.12)", color:"#34d399" };
...
function StatBox({ label, value, color="#a5b4fc" }) {
  return (
    <div style={{background:surf,border:`1px solid ${bdr}`,borderRadius:12,padding:"14px 18px",flex:1,minWidth:120}}>
      <div style={{fontSize:11,color:mut,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>{label}</div>
...
// ── Card Detail Page ──────────────────────────────────────────────────────────
function CardDetail({ card, db, onBack }) {
  ...
  const load = async () => {
    ...
    const [t, p] = await Promise.all([
      db.from("point_transactions").filter("program_name", card.name),
      db.from("transfer_partners").filter("from_program", card.name),
    ]);
  ...
  const saveTxn = async () => {
    ...
    const {error} = await db.from("point_transactions").insert({
      program_name: card.name, program_type:"card",
      points: pts, description: f.description, txn_date: f.txn_date
    });
    if (error) return alert("Error: "+JSON.stringify(error));
    // update card balance
    await db.from("cc_cards").update(card.id, { points_balance: (card.points_balance||0) + pts });
    card.points_balance = (card.points_balance||0) + pts;
    ...
    <div>
      ...
      {partners.length>0&&(
        <div style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"18px 20px",marginBottom:20}}>
          <div style={{fontSize:13,fontWeight:700,color:"#a5b4fc",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:14}}>Transfer Partners</div>
      ...
      <div style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"18px 20px"}}>
        ...
        : txns.length===0 ? <div style={{color:mut,fontSize:13,textAlign:"center",padding:20}}>No transactions yet</div>
        : (
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead>
                <tr style={{color:mut,fontSize:11,textTransform:"uppercase",letterSpacing:"0.05em"}}>
                  <th style={{textAlign:"left",padding:"8px 12px",borderBottom:`1px solid ${bdr}`}}>Date</th>
                  <th style={{textAlign:"left",padding:"8px 12px",borderBottom:`1px solid ${bdr}`}}>Description</th>
                  <th style={{textAlign:"right",padding:"8px 12px",borderBottom:`1px solid ${bdr}`}}>Points</th>
                  <th style={{textAlign:"right",padding:"8px 12px",borderBottom:`1px solid ${bdr}`}}>Opening</th>
                  <th style={{textAlign:"right",padding:"8px 12px",borderBottom:`1px solid ${bdr}`}}>Closing</th>
                </tr>
              </thead>
              <tbody>
                {txnsWithBalance().map(t=>(
                  <tr key={t.id} style={{borderBottom:`1px solid ${bdr}`}}>
                    <td style={{padding:"10px 12px",color:mut}}>{new Date(t.txn_date).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"})}</td>
      ...
      <Modal show={showTxn} onClose={()=>setShowTxn(false)} title="Add Transaction">
        {lbl("Type")}
        <select style={inp} value={f.type} onChange={up("type")}>
...
// ── LP Detail Page ────────────────────────────────────────────────────────────
function LPDetail({ prog, db, onBack }) {
  const [txns, setTxns] = useState([]);
  const [partners, setPartners] = useState([]);
  const [busy, setBusy] = useState(true);
  const [showTxn, setShowTxn] = useState(false);
  ...
  const load = async () => {
    setBusy(true);
    const [t, p] = await Promise.all([
      db.from("point_transactions").filter("program_name", prog.name),
      db.from("transfer_partners").filter("from_program", prog.name),
    ]);
    setTxns((t.data||[]).sort((a,b)=>new Date(b.txn_date)-new Date(a.txn_date)));
    setPartners(p.data||[]);
    setBusy(false);
  ...
  const saveTxn = async () => {
    ...
    const pts = f.type==="redeem" ? -Math.abs(parseInt(f.points)) : Math.abs(parseInt(f.points));
    const {error} = await db.from("point_transactions").insert({
      program_name: prog.name, program_type:"loyalty",
      points: pts, description: f.description, txn_date: f.txn_date
    });
    if (error) return alert("Error: "+JSON.stringify(error));
    await db.from("loyalty_programs").update(prog.id, { points_balance: (prog.points_balance||0) + pts });
    prog.points_balance = (prog.points_balance||0) + pts;
    setShowTxn(false); setF(tf); load();
  };
    ...
    <div>
      ...
      <div style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"20px 22px",borderTop:`4px solid ${prog.color||"#8b5cf6"}`,marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
          <div>
            ...
            <div style={{fontSize:13,color:mut,marginTop:3}}>
              <span style={{color:cc[prog.category]||"#8b5cf6"}}>{prog.category}</span>
              {prog.tier&&<span style={{color:"#a78bfa"}}> · {prog.tier}</span>}
            </div>
          </div>
          <button style={pbtn} onClick={()=>{ setF(tf); setShowTxn(true); }}>+ Add Transaction</button>
        </div>
        <div style={{display:"flex",gap:12,marginTop:16,flexWrap:"wrap"}}>
          ...
          {prog.expiry_date&&<StatBox label="Expiry" value={exp?`⚠ ${d}d left`:`${d}d left`} color={exp?"#f87171":"#a5b4fc"}/>}
        </div>
...
function Cards({ db }) {
  ...
  if (detail) return <CardDetail card={detail} db={db} onBack={()=>{ setDetail(null); load(); }}/>;

  const openAdd=()=>{ setEdit(null); setF({...empty}); setShow(true); };
  const openEdit=(r,e)=>{ e.stopPropagation(); setEdit(r); setF({name:r.name||"",bank:r.bank||"",last4:r.last4||"",network:r.network||"Visa",points_balance:String(r.points_balance||""),points_currency:r.points_currency||"pts",stmt_date:String(r.stmt_date||""),annual_fee:String(r.annual_fee||""),color:r.color||"#6366f1"}); setShow(true); };
  ...
  const del=async(id,e)=>{ e.stopPropagation(); if(confirm("Delete?")){ await db.from("cc_cards").delete(id); load(); } };
    ...
    <div>
      ...
      {busy?<div style={{color:mut,padding:40,textAlign:"center"}}>Loading…</div>:rows.length===0?(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
          {rows.map(c=>(
            <div key={c.id} onClick={()=>setDetail(c)} style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"18px 20px",borderTop:`3px solid ${c.color||acc}`,position:"relative",cursor:"pointer",transition:"border-color 0.15s"}}
              onMouseEnter={e=>e.currentTarget.style.borderColor=c.color||acc}
              ...
              <div style={{position:"absolute",top:12,right:12,display:"flex",gap:4}}>
                <button style={{...gbtn,padding:"5px 8px"}} onClick={e=>openEdit(c,e)}>✎</button>
                <button style={{...dbtn,padding:"5px 8px"}} onClick={e=>del(c.id,e)}>✕</button>
              </div>
...
function Loyalty({ db }) {
  ...
  if (detail) return <LPDetail prog={detail} db={db} onBack={()=>{ setDetail(null); load(); }}/>;

  const openAdd=()=>{ setEdit(null); setF({...empty}); setShow(true); };
  const openEdit=(r,e)=>{ e.stopPropagation(); setEdit(r); setF({name:r.name||"",category:r.category||"Airline",points_balance:String(r.points_balance||""),expiry_date:r.expiry_date||"",expiry_rule:r.expiry_rule||"",tier:r.tier||"",color:r.color||"#8b5cf6"}); setShow(true); };
  ...
  const del=async(id,e)=>{ e.stopPropagation(); if(confirm("Delete?")){ await db.from("loyalty_programs").delete(id); load(); } };
  const days=d=>d?Math.round((new Date(d)-new Date())/86400000):null;
      ...
      {busy?<div style={{color:mut,padding:40,textAlign:"center"}}>Loading…</div>:rows.length===0?(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
          {rows.map(p=>{
            ...
            return (
              <div key={p.id} onClick={()=>setDetail(p)} style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"18px 20px",borderTop:`3px solid ${p.color||"#8b5cf6"}`,position:"relative",cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                ...
                <div style={{position:"absolute",top:12,right:12,display:"flex",gap:4}}>
                  <button style={{...gbtn,padding:"5px 8px"}} onClick={e=>openEdit(p,e)}>✎</button>
                  <button style={{...dbtn,padding:"5px 8px"}} onClick={e=>del(p.id,e)}>✕</button>
                </div>
...
function TransferPoints({ db }) {
  ...
  // Partners where from is the from_program
  const validTos = from ? allPartners.filter(p=>p.from_program===from).map(p=>p.to_program) : [];
  // Partners where to is the from_program (reverse lookup)
  const validFroms = to ? allPartners.filter(p=>p.to_program===to).map(p=>p.from_program) : [];

  const partner = from&&to ? allPartners.find(p=>p.from_program===from&&p.to_program===to) : null;
  const received = partner&&pts ? Math.floor((parseInt(pts)||0) * (partner.ratio_to/partner.ratio_from)) : null;

  const fromProg = allPrograms.find(p=>p.name===from);
  const toProg = allPrograms.find(p=>p.name===to);

  const doTransfer = async () => {
    if (!from||!to||!pts) return alert("Fill all fields");
    if (!partner) return alert("No transfer route found");
    const sendPts = parseInt(pts);
    if (sendPts<=0) return alert("Enter valid points");
    if (partner.min_transfer&&sendPts<partner.min_transfer) return alert(`Minimum transfer is ${partner.min_transfer.toLocaleString()} pts`);

    // Deduct from source
    const fromObj = fromProg.type==="card"
      ? cards.find(c=>c.name===from)
      : loyalties.find(l=>l.name===from);
    const toObj = toProg.type==="card"
      ? cards.find(c=>c.name===to)
      : loyalties.find(l=>l.name===to);

    const table_from = fromProg.type==="card" ? "cc_cards" : "loyalty_programs";
    const table_to = toProg.type==="card" ? "cc_cards" : "loyalty_programs";
    const today = new Date().toISOString().split("T")[0];

    await db.from(table_from).update(fromObj.id, { points_balance: (fromObj.points_balance||0) - sendPts });
    await db.from(table_to).update(toObj.id, { points_balance: (toObj.points_balance||0) + received });

    // Log transactions
    await db.from("point_transactions").insert({ program_name:from, program_type:fromProg.type, points:-sendPts, description:`Transfer to ${to}`, txn_date:today });
    await db.from("point_transactions").insert({ program_name:to, program_type:toProg.type, points:received, description:`Transfer from ${from}`, txn_date:today });

    setDone({ from, to, sent:sendPts, received });
    setFrom(""); setTo(""); setPts("");
  };
  ...
  return (
    <div>
      ...
      {done&&(
        <div style={{background:"rgba(52,211,153,0.08)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:12,padding:"14px 18px",marginBottom:20,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <div style={{fontSize:13,color:"#34d399",fontWeight:600}}>✓ Transfer complete!</div>
          <div style={{fontSize:13,color:txt}}>{done.sent.toLocaleString()} pts from <strong>{done.from}</strong> → {done.received.toLocaleString()} pts to <strong>{done.to}</strong></div>
          <button onClick={()=>setDone(null)} style={{background:"none",border:"none",cursor:"pointer",color:mut,fontSize:18}}>×</button>
        </div>
      )}

      <div style={{background:surf,border:`1px solid ${bdr}`,borderRadius:14,padding:"22px 22px",maxWidth:560}}>
        ...
        <div style={{marginBottom:20}}>
          ...
          {lbl("Program")}
          <select style={inp} value={from} onChange={e=>{ setFrom(e.target.value); setTo(""); }}>
            <option value="">— select source —</option>
            {allPrograms.filter(p=>to ? validFroms.includes(p.name)||!to : allPartners.some(r=>r.from_program===p.name)).map(p=>(
              <option key={p.name} value={p.name}>{p.name} ({p.balance.toLocaleString()} pts)</option>
            ))}
          </select>
          {fromProg&&<div style={{fontSize:12,color:mut,marginTop:-8,marginBottom:12}}>Available: <span style={{color:"#fff",fontWeight:600}}>{fromProg.balance.toLocaleString()} pts</span></div>}
        </div>
        ...
        <div style={{marginBottom:20}}>
          ...
          {lbl("Program")}
          <select style={inp} value={to} onChange={e=>setTo(e.target.value)} disabled={!from}>
            <option value="">— select destination —</option>
            {validTos.map(name=>{
              const p=allPrograms.find(x=>x.name===name);
              return <option key={name} value={name}>{name}{p?` (${p.balance.toLocaleString()} pts)`:""}</option>;
            })}
          </select>
          {!from&&<div style={{fontSize:11,color:mut,marginTop:-8,marginBottom:12}}>Select source first</div>}
          {toProg&&<div style={{fontSize:12,color:mut,marginTop:-8,marginBottom:12}}>Current balance: <span style={{color:"#fff",fontWeight:600}}>{toProg.balance.toLocaleString()} pts</span></div>}
        </div>

        {/* Partner info */}
        {partner&&(
          <div style={{background:"rgba(99,102,241,0.07)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:10,padding:"12px 16px",marginBottom:20}}>
            <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
              <div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>Ratio</div><div style={{fontSize:16,fontWeight:800,color:"#fff"}}>{partner.ratio_from}:{partner.ratio_to}</div></div>
              {partner.min_transfer&&<div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>Min Transfer</div><div style={{fontSize:16,fontWeight:800,color:"#a5b4fc"}}>{partner.min_transfer.toLocaleString()}</div></div>}
              {partner.max_monthly&&<div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>Max/Month</div><div style={{fontSize:16,fontWeight:800,color:"#a5b4fc"}}>{partner.max_monthly.toLocaleString()}</div></div>}
              {partner.transfer_time&&<div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>Transfer Time</div><div style={{fontSize:16,fontWeight:800,color:"#34d399"}}>{partner.transfer_time}</div></div>}
            </div>
          </div>
        )}
        ...
        {/* Points input */}
        <div style={{marginBottom:20}}>
          {lbl("Points to Transfer")}
          <input style={inp} type="number" placeholder="Enter points" value={pts} onChange={e=>setPts(e.target.value)} disabled={!partner}/>
          {partner&&pts&&received!==null&&(
            <div style={{background:"rgba(52,211,153,0.07)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:10,padding:"12px 16px",marginTop:-8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                <div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>You send</div><div style={{fontSize:20,fontWeight:800,color:"#f87171"}}>{parseInt(pts).toLocaleString()} pts</div></div>
                <div style={{fontSize:20,color:mut}}>→</div>
                <div><div style={{fontSize:11,color:mut,textTransform:"uppercase"}}>They receive</div><div style={{fontSize:20,fontWeight:800,color:"#34d399"}}>{received.toLocaleString()} pts</div></div>
              </div>
            </div>
          )}
        </div>
...
function Transfers({ db }) {
  ...
  const load=async()=>{
    setBusy(true);
    const [t,c,l]=await Promise.all([db.from("transfer_partners").select(),db.from("cc_cards").select(),db.from("loyalty_programs").select()]);
    setRows(t.data); setCards(c.data); setLoyalties(l.data); setBusy(false);
  };
  useEffect(()=>{ load(); },[]);
    ...
    <div>
      ...
      <Modal show={show} onClose={()=>setShow(false)} title={edit?"Edit Route":"Add Transfer Route"}>
        <div style={{background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:10,padding:14,marginBottom:14}}>
          <div style={{fontSize:11,color:acc,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>From</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>{lbl("Type")}<select style={inp} value={f.from_type} onChange={e=>setF(p=>({...p,from_type:e.target.value,from_program:""}))}>
              <option value="card">Credit Card</option><option value="loyalty">Loyalty Program</option>
            </select></div>
            <div>{lbl("Program")}<select style={inp} value={f.from_program} onChange={up("from_program")}>
              <option value="">— select —</option>{optionsFor(f.from_type).map(o=><option key={o} value={o}>{o}</option>)}
            </select></div>
          </div>
        </div>
        <div style={{background:"rgba(52,211,153,0.06)",border:"1px solid rgba(52,211,153,0.15)",borderRadius:10,padding:14,marginBottom:14}}>
          <div style={{fontSize:11,color:"#34d399",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>To</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>{lbl("Type")}<select style={inp} value={f.to_type} onChange={e=>setF(p=>({...p,to_type:e.target.value,to_program:""}))}>
              <option value="card">Credit Card</option><option value="loyalty">Loyalty Program</option>
            </select></div>
            <div>{lbl("Program")}<select style={inp} value={f.to_program} onChange={up("to_program")}>
              <option value="">— select —</option>{optionsFor(f.to_type).map(o=><option key={o} value={o}>{o}</option>)}
            </select></div>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
    ...
    <div style={{minHeight:"100vh",background:bg,color:txt,fontFamily:"Inter,system-ui,sans-serif",fontSize:14}}>
      <style>{`
        @media(max-width:640px){
          ...
          .main-wrap{margin-left:0!important;padding:16px!important;padding-top:68px!important}
        }
